...
public class GraphWidget implements AnnotatedWidget {
  ...
  private static final Logger log = Logger.getLogger(edu.wpi.first.shuffleboard.plugin.base.widget.GraphWidget.class.getName());
   ...
   */
  private static final Collection<edu.wpi.first.shuffleboard.plugin.base.widget.GraphWidget> graphWidgets =
  ...
  private static final Future<?> updater = ThreadUtils.newDaemonScheduledExecutorService()
      .scheduleAtFixedRate(() -> {
        synchronized (graphWidgets) {
          graphWidgets.forEach(edu.wpi.first.shuffleboard.plugin.base.widget.GraphWidget::update);
        }
